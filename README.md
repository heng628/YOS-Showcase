...
| 🔌 **数据接入**（规划中） | 常用文件导入（PDF / PPT / Word 等，近期）；平台数据接入：**B站创作者中心 / 抖音 / 淘宝 / 小红书 / 闲鱼**（S2 首发 5 平台，用户授权自己的账号）；**通用 API / 接口窗口**（任何可调用的接口，用户填入凭证与参数即可接入）；定时同步与文件监控 |
| 🦾 **双执行面 Agent 架构**（设计定案） | ① **确定性执行面**：接入/解析/指标衍生/调度由程序引擎完成（用户自己管理、无记忆）；② **智能理解面**：AI 理解/解释/建议/归因，带可成长记忆与画像（用户可见、可删、可纠正）——LLM 不参与计算、永不生成 SQL |
...
1. **S1 验证**：单机 MVP 用户验证（当前阶段）；**扩展常用文件导入：PDF / PPT / Word 等**（近期）
2. **S2 能力面**：**数据源中心**——首发平台接入（B站创作者中心 / 抖音 / 淘宝 / 小红书 / 闲鱼）+ **通用 API / 接口窗口**（用户输入凭证与参数即可创建可调用数据源）+ 定时同步；云同步/备份/计量；正式报告中心；**Agent 记忆与学习**（证据驱动的确定性记忆 + 用户画像，用户可见可删）；Agent 写能力（propose→确认→执行）
